...
public class AnimalGeneticsSystem extends BaseComponentSystem {
    ...
    public void onMatingStart(MatingInitiatedEvent event, EntityRef entityRef, MatingComponent matingComponent) {
        ...
        for (int i = 0; i <= this.getSiblings(); i++) {
            if (event.animal1.getParentPrefab().getName().equals("WildAnimals:deer")) {
                ...
                entityRef.send(new OnBreed(event.animal1, event.animal2,
                        entityManager.create(event.animal1.getParentPrefab())));
            }
        }
    ...
    /**
     * This method processes the number of siblings according to the following probability scale: Chances of having
     * non-identical twins: 4.5% Chances of having non-identical triplets: 0.5%
     */
